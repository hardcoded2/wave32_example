	...
	class HandRenderer : MonoBehaviour
	{
		...
		private int GetGestureIndex()
		{
			ulong gesture_value = this.IsLeft ?
				WaveVR_GestureManager.Instance.GetCurrentLeftHandStaticGesture() :
				WaveVR_GestureManager.Instance.GetCurrentRightHandStaticGesture();

			if ((gesture_value & ((ulong)WaveVR_GestureManager.EStaticGestures.UNKNOWN)) != 0)
				return 1;
			if ((gesture_value & ((ulong)WaveVR_GestureManager.EStaticGestures.FIST)) != 0)
				return 2;
			if ((gesture_value & ((ulong)WaveVR_GestureManager.EStaticGestures.FIVE)) != 0)
				return 3;
			if ((gesture_value & ((ulong)WaveVR_GestureManager.EStaticGestures.OK)) != 0)
				return 4;
			if ((gesture_value & ((ulong)WaveVR_GestureManager.EStaticGestures.THUMBUP)) != 0)
				return 5;
			if ((gesture_value & ((ulong)WaveVR_GestureManager.EStaticGestures.INDEXUP)) != 0)
				return 6;
		...
		IEnumerator Start()
		{
			...
			while (!WaveVR.Instance.IsHandTrackingEnabled())
				yield return null;
		...
		void Update()
		{
			...
			bool valid_pose = IWaveVR_BonePose.Instance.IsHandPoseValid((this.IsLeft ? WaveVR_GestureManager.EGestureHand.LEFT : WaveVR_GestureManager.EGestureHand.RIGHT));
			...
			transform.position = IWaveVR_BonePose.Instance.GetBoneTransform(0, this.IsLeft).pos;// hand.position;
			transform.rotation = IWaveVR_BonePose.Instance.GetBoneTransform(0, this.IsLeft).rot;// hand.rotation;
			...
			if (showConficenceAsAlpha)
			{
				...
				float hand_confidence = IWaveVR_BonePose.Instance.GetHandConfidence((this.IsLeft ? WaveVR_GestureManager.EGestureHand.LEFT : WaveVR_GestureManager.EGestureHand.RIGHT));
			...
			{
				var go = points[i];
				go.transform.position = IWaveVR_BonePose.Instance.GetBoneTransform(i, this.IsLeft).pos;// hand.points[i];
			...
			for (int i = 0; i < links.Count; i++)
			{
				...
				var pose1 = IWaveVR_BonePose.Instance.GetBoneTransform(startIndex, this.IsLeft).pos;// hand.points[startIndex];
				if (!IsValidGesturePoint(pose1)/*pose1.IsValidGesturePoint()*/)
					continue;

				var pose2 = IWaveVR_BonePose.Instance.GetBoneTransform(Connections[i * 2 + 1], this.IsLeft).pos;// hand.points[Connections[i * 2 + 1]];
