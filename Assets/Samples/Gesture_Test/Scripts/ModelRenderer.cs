...
namespace WaveVRGesture
{
	...
	public class ModelRenderer : MonoBehaviour
	{
		...
		void Update()
		{
			transform.position = IWaveVR_BonePose.Instance.GetBoneTransform(0, this.IsLeft).pos;// result.points[0];
			transform.rotation = /*result.rotation*/IWaveVR_BonePose.Instance.GetBoneTransform(0, this.IsLeft).rot * Quaternion.Euler(initialRotation);

			Vector3 indexDir =
				IWaveVR_BonePose.Instance.GetBoneTransform(5, this.IsLeft).pos -
				IWaveVR_BonePose.Instance.GetBoneTransform(0, this.IsLeft).pos;// result.points[5] - result.points[0];
			Vector3 midDir =
				IWaveVR_BonePose.Instance.GetBoneTransform(9, this.IsLeft).pos -
				IWaveVR_BonePose.Instance.GetBoneTransform(0, this.IsLeft).pos;//result.points[9] - result.points[0];

			Vector3 palmDir = IsLeft ? Vector3.Cross(indexDir, midDir) : Vector3.Cross(midDir, indexDir);
			Vector3 thumbAxis =
				IWaveVR_BonePose.Instance.GetBoneTransform(17, this.IsLeft).pos -
				IWaveVR_BonePose.Instance.GetBoneTransform(1, this.IsLeft).pos;//result.points[17] - result.points[1];
			...
			{
				Vector3 root = IWaveVR_BonePose.Instance.GetBoneTransform(nodeIndex, this.IsLeft).pos;//result.points[nodeIndex];
				Vector3 joint1 = IWaveVR_BonePose.Instance.GetBoneTransform(nodeIndex + 1, this.IsLeft).pos;//result.points[nodeIndex + 1];
				Vector3 vec1 = (joint1 - root).normalized * modelFingerVec[vecIndex].magnitude;
				Vector3 joint2 = IWaveVR_BonePose.Instance.GetBoneTransform(nodeIndex + 2, this.IsLeft).pos;//result.points[nodeIndex + 2];
				Vector3 vec2 = (joint2 - joint1).normalized * modelFingerVec[vecIndex + 1].magnitude;
				Vector3 top = IWaveVR_BonePose.Instance.GetBoneTransform(nodeIndex + 3, this.IsLeft).pos;//result.points[nodeIndex + 3];
			...
			if (showConficenceAsAlpha)
			{
				...
				float hand_confidence = IWaveVR_BonePose.Instance.GetHandConfidence((IsLeft ? WaveVR_GestureManager.EGestureHand.LEFT : WaveVR_GestureManager.EGestureHand.RIGHT));
