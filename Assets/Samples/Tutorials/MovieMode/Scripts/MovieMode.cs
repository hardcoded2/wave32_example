...
public class MovieMode : MonoBehaviour {
	...
	class MatrixSpline
	{
		public class MatrixData
		{
			...
			{
				pos = WaveVR_Utils.GetPosition(mat);
				rot = WaveVR_Utils.GetRotation(mat);
			}
	...
	void UpdatePose2(object[] args)
	{
		...
		if (lockHMDPosition && needSmoothMove)
		{
			...
			var pos = Vector3.Lerp(WaveVR_Utils.GetPosition(from), WaveVR_Utils.GetPosition(to), time / (float)total);
			var rot = Quaternion.Lerp(WaveVR_Utils.GetRotation(from), WaveVR_Utils.GetRotation(to), time / (float)total);
