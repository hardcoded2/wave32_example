...
public class BuildHelloVR
{
	...
	private static void GeneralSettingsWindows()
	{
		PlayerSettings.bundleVersion = "3.0.0";
		PlayerSettings.companyName = "HTC Corp.";
		PlayerSettings.defaultInterfaceOrientation = UIOrientation.LandscapeLeft;
	...
	[UnityEditor.MenuItem("WaveVR/Build Windows Exe/Build+Run wvr_unity_hellovr.exe")]
	static void BuildAndRunExe()
	{
		string[] levels = { "Assets/Samples/HelloVR/Scenes/HelloVR.unity" };

		BuildExeInner("hellovr.unity", "wvr_unity_hellovr.exe", null, true, levels);
	}
	...
	private static void BuildExeInner(string idName, string apkName, string destPath, bool run, string[] levels)
	{
		GeneralSettingsWindows();

		PlayerSettings.productName = "HelloVR";
#if UNITY_5_6_OR_NEWER
		PlayerSettings.SetApplicationIdentifier(BuildTargetGroup.Standalone, "com.htc.vr.samples." + idName);
#else
		...
		BuildPipeline.BuildPlayer(levels, outputFilePath, BuildTarget.StandaloneWindows64, run ? BuildOptions.AutoRunPlayer : BuildOptions.None);
	}
