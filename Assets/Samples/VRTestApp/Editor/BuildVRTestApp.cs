...
public class BuildVRTestApp
{
	...
	[UnityEditor.MenuItem("WaveVR/Build Android APK/Build Only Script", false, 1)]
	static void CheckerBuildOnlyScript()
	{
		isBuildOnlyScript = !isBuildOnlyScript;
		Menu.SetChecked("WaveVR/Build Android APK/Build Only Script", isBuildOnlyScript);
	}
	...
	[UnityEditor.MenuItem("WaveVR/Build Windows Exe/Build+Run wvr_unity_vrtestapp.exe")]
	...
	public static void BuildExe(string destPath, bool run)
	{
		...
		BuildApkInnerWindows(destPath, run, levels);
	}

		private static string[] ApkLevels = {
			"Assets/Samples/VRTestApp/scenes/VRTestApp.unity",
			"Assets/Samples/SeaOfCube/scenes/Main.unity",
			"Assets/Samples/SeaOfCube/scenes/Main_Help.unity",
			"Assets/Samples/SeaOfCube/scenes/SeaOfCubeWithTwoHead.unity",
			...
			"Assets/Samples/AdaptiveQualityDynamicResolution_Test/Scenes/AQDR_Loading.unity",
		};
	...
	public static void BuildApk(string destPath, bool run, bool development, bool isIL2CPP = false, bool isSupport32 = true, bool isSupport64 = false)
	{
		...
		for (int i =0; i < ApkLevels.Length;i++)
		{
			if (File.Exists(ApkLevels[i])) {
				tmplist.Add(ApkLevels[i]);
			}
			else
			{
				Debug.Log("Skip Path not existed : " + ApkLevels[i]);
			}
		}
		string[] levels = tmplist.ToArray();
